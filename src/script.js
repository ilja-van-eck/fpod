document.addEventListener("DOMContentLoaded", () => {
  ...
  function initSlider() {
    ...
    boxes.forEach((box) => {
      box.addEventListener("click", () => {
        ...
        tl.set(overlayWrap, { display: "block" })
          ...
          .to(".navbar-wrap", { y: "-5rem" }, "<+=0.1");

        let imageState = Flip.getState(image);
        let infoState = Flip.getState(info);
    ...
    overlayClose.addEventListener("click", () => {
      ...
      let info = overlayWrap.querySelector(".slide-info__w");
      let infoTarget = originalSlide.querySelector(".slide-info");
      ...
      tl.to(overlayScroll, { autoAlpha: 0 }, 0)
        .to(overlayLine, { scaleX: 0 }, "<")
        .to(overlayClose, { scale: 0, rotate: -25 }, "<")
        .to(".navbar-wrap", { y: "0rem" }, "<+=0.2")
    ...
    if (!isMobile) {
      wrapper.addEventListener("mouseenter", () => {
        gsap.set(cursorInner, { display: "flex" });
      });
